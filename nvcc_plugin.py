    ...
    def cu(self, line, cell):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with open(file_path + ext, "w") as f:
                f.write(cell)
            try:
                subprocess.check_output([compiler, '-I/usr/local/cuda/samples/common/inc/', '-I/usr/local/cuda/include', '-L/usr/local/cuda/lib64', '-lcublas', '-lcusolver', '-lcudnn', '-lcufft', '-lcurand', '-lcusparse', '-lcutensor', '--gpu-architecture=sm_75', '-Wno-deprecated-gpu-targets', '-Xcompiler="-fopenmp"', '-Xcompiler="-pthread"', '-Xcompiler="-lgomp"', '-Xcompiler="-lopenblas64"', file_path + ext, "-o", file_path + ".out"], stderr=subprocess.STDOUT)
